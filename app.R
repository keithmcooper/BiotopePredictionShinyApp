...
server <- function(input, output) {
  ...
  output$downloadTemp <- downloadHandler(
    ...
    content = function(file) {
      myfile <- paste0('www/',"ShinyTemplate.csv", collapse = NULL)
  ...
  output$distances <- DT::renderDataTable({
    ...
    if ( !is.null(res$k_st) )  {
      ...
      #convert results and shinytempalte4 into dataframes
       
      
      results_df <- as.data.frame(results$centers)
      shinytemplate_df<- as.data.frame(ShinyTemplate4)
      #match names of one dataframe with the othger to be able to run rbind function ( nnesd matching names)  
      
      names(results_df) <- names ( shinytemplate_df)
      
      DistancestoCentersTest <- as.matrix(dist(rbind( results_df, shinytemplate_df )))[-(1:12),1:12]
